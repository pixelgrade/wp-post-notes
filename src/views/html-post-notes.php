...
<ul class="post_notes">
	...
	if ( $notes ) {
		foreach ( $notes as $note ) {
			...
			$css_class   = apply_filters( 'pixelgrade_wppostnotes/note_class', array_filter( $css_class ), $note );
			?>
			<li rel="<?php echo absint( $note->id ); ?>" class="<?php echo esc_attr( implode( ' ', $css_class ) ); ?>">
				...
				<p class="meta">
					<abbr class="exact-date" title="<?php echo esc_attr( $note->date_created->format( 'Y-m-d H:i:s' ) ); ?>">
						...
						echo esc_html( sprintf( __( '%1$s at %2$s', 'woocommerce' ),  wp_date(  Pixelgrade\WPPostNotes\get_date_format(), $note->date_created->format('U') ), wp_date(  Pixelgrade\WPPostNotes\get_time_format(), $note->date_created->format('U') ) ) );
						?>
					</abbr>
					<?php
